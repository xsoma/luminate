...
void aim::fire(CUserCmd* cmd)
{
	...
	if (hitchance_amount) {
		...
		if (final_hitchance < g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].hitchance_amount)
		{
			...
			if ((int)g_ctx.globals.eye_pos.DistTo(final_target.record->origin) < g_cfg.ragebot.autoscope_distance_float && g_cfg.ragebot.autoscope_distance) {
	...
	if (net_channel_info)
	{
		...
		if (g_cfg.ragebot.double_tap && g_cfg.ragebot.double_tap_key.key > KEY_NONE && g_cfg.ragebot.double_tap_key.key < KEY_MAX && misc::get().double_tap_key)
		{
			...
			{
				original_tickbase += g_ctx.globals.weapon->get_max_tickbase_shift();
			}
		}
		...
		if (g_cfg.antiaim.hide_shots && g_cfg.antiaim.hide_shots_key.key > KEY_NONE && g_cfg.antiaim.hide_shots_key.key < KEY_MAX && misc::get().hide_shots_key)
		{
			...
			{
				original_tickbase += m_gamerules()->m_bIsValveDS() ? 6 : 9;
			}
		}
...
int aim::hitchance(const Vector& aim_angle)
{
	...
	for (auto i = 1; i <= 6; ++i)
	{
		for (auto j = 0; j < 8; ++j)
		{
			...
			if (hitbox_intersection(final_target.record->player, final_target.record->matrixes_data.main, final_target.data.hitbox, g_ctx.globals.eye_pos, end))
			{
				auto fire_data = c_autowall::get().calculate_return_info(g_ctx.globals.eye_pos, end, g_ctx.local(), final_target.record->player);
				auto valid_hitbox = true;

				if (final_target.data.hitbox == HITBOX_HEAD && fire_data.m_hitbox != HITBOX_HEAD)
					valid_hitbox = false;

				if (fire_data.m_damage >= 1 && valid_hitbox)
					damage += high_accuracy_weapon ? fire_data.m_damage : 1;
			}
		}
	}
	...
	return (float)damage / 48.0f >= (float)g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].accuracy_boost_amount * 0.01f ? final_hitchance : 0;*/
}
