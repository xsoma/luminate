...
void misc::watermark()
{
	...
	auto watermark = crypt_str("luminate | ") + g_ctx.username /*+ crypt_str(" | ")*/ + g_ctx.globals.time;
	...
	if (m_engine()->IsInGame())
	{
		...
		if (nci)
		{
			...
			watermark = crypt_str("luminate | ") + g_ctx.username /*+ crypt_str(" | ")*/ + server + crypt_str(" | ") + std::to_string(g_ctx.globals.ping) + crypt_str(" ms | ") + tickrate + crypt_str(" tick | ") + g_ctx.globals.time;
		}
	}
...
bool misc::double_tap(CUserCmd* m_pcmd)
{
	...
	if (!g_ctx.globals.weapon->is_grenade() && g_ctx.globals.weapon->m_iItemDefinitionIndex() != WEAPON_TASER && g_ctx.globals.weapon->m_iItemDefinitionIndex() != WEAPON_REVOLVER && g_ctx.send_packet && (m_pcmd->m_buttons & IN_ATTACK || m_pcmd->m_buttons & IN_ATTACK2 && g_ctx.globals.weapon->is_knife()))
	{
		...
		shitfted_comm = user_cmd->m_command_number;

		util::copy_command(user_cmd, g_ctx.globals.weapon->get_max_tickbase_shift());
