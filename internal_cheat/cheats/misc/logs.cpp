...
void eventlogs::events(IGameEvent* event) 
{
	...
	if (g_cfg.misc.events_to_log[EVENTLOG_HIT] && !strcmp(event->GetName(), crypt_str("player_hurt")))
	{
		...
		{
			ss << crypt_str("hurt ") << userid_info.szName << crypt_str(" for ") << event->GetInt(crypt_str("dmg_health")) << crypt_str(" in ") << get_hitgroup_name(event->GetInt(crypt_str("hitgroup")));
		
			add(ss.str());
		}
		else if (userid_id == m_engine()->GetLocalPlayer() && attacker_id != m_engine()->GetLocalPlayer()) 
		{
			ss << crypt_str("harmed by ") << attacker_info.szName << crypt_str(" for ") << event->GetInt(crypt_str("dmg_health")) << crypt_str(" in ") << get_hitgroup_name(event->GetInt(crypt_str("hitgroup")));
...
void eventlogs::add(std::string text, bool full_display)
{
	...
	if (g_cfg.misc.log_output[EVENTLOG_OUTPUT_CONSOLE])
	{
		...
		m_cvar()->ConsoleColorPrintf(Color(89,119,239,255), crypt_str("[ luminate-project ] "));
	...
	if (g_cfg.misc.log_output[EVENTLOG_OUTPUT_CHAT])
	{
		...
		auto log = crypt_str("[ \x0C luminate-project \x01] ") + text;
