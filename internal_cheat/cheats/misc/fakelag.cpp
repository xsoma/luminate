...
void fakelag::Fakelag(CUserCmd* m_pcmd)
{
	...
	else
	{
		...
		else if (!g_ctx.globals.exploits && g_cfg.antiaim.fakelag && g_cfg.antiaim.fakelag_enablers[FAKELAG_SHOT] && g_ctx.globals.aimbot_working) {
			if (choked < max_choke)
				g_ctx.send_packet = false;
			else
			{
		...
		else if (!g_ctx.globals.exploits && g_cfg.antiaim.fakelag && g_cfg.antiaim.fakelag_enablers[FAKELAG_STAND] && g_ctx.local()->m_vecVelocity().Length() <= 5.f) {
			if (choked < max_choke)
				g_ctx.send_packet = false;
			else
			{
