...
void otheresp::penetration_reticle()
{
	...
	m_engine()->GetScreenSize(screen_width, screen_height);

	static int width = screen_width / 2;
	static int height = screen_height / 2;

	render::get().line(width - 20, height - 20, width - 20, height + 20, color);
	render::get().line(width - 20, height + 20, width + 20, height + 20, color);
	render::get().line(width - 20, height - 20, width + 20, height - 20, color);
	render::get().line(width + 20, height + 20, width + 20, height - 20, color);
}
...
void otheresp::draw_indicators()
{
	...
	for (size_t i{}; i < m_indicators.size(); i++)
	{
		auto indicator = m_indicators[i];
		auto text_width = render::get().text_width(fonts[INDICATORFONT], indicator.m_text.c_str());

		if (g_cfg.esp.penetration_reticle)
			render::get().text(fonts[INDICATORFONT], width * 0.5f - text_width * 0.5f, height * 0.5f + 30 + (14 * i), indicator.m_color, HFONT_CENTERED_Y, indicator.m_text.c_str());
		else
			render::get().text(fonts[INDICATORFONT], width * 0.5f - text_width * 0.5f, height * 0.5f + 20 + (14 * i), indicator.m_color, HFONT_CENTERED_Y, indicator.m_text.c_str());
	}
