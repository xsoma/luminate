...
void playeresp::draw_box(player_t* m_entity, const Box& box)
{
	...
	const float calc_horisontal = (g_cfg.player.type[type].box_horisontal * 0.01f) * 0.5f;
	const float calc_vertical = (g_cfg.player.type[type].box_vertical * 0.01f) * 0.5f;
	const float offset_horisontal = g_cfg.player.type[type].box_horisontal >= 99.f ? 1 : 0;
	const float offsest_vertical = g_cfg.player.type[type].box_vertical >= 99.f ? 1 : 0;

	render::get().line(box.x, box.y, offset_horisontal + box.x + box.w * calc_horisontal, box.y, color);
	render::get().line(box.x + box.w - 1, box.y, box.x + box.w * (1.f - calc_horisontal) - 1, box.y, color);
	render::get().line(box.x, box.y + box.h - 1, offset_horisontal + box.x + box.w * calc_horisontal, box.y + box.h - 1, color);
	render::get().line(box.x + box.w - 1, box.y + box.h - 1, box.x + box.w * (1.f - calc_horisontal) - 1, box.y + box.h - 1, color);

	//outline
	render::get().line(box.x - 1, box.y - 1, offset_horisontal + box.x + box.w * calc_horisontal, box.y - 1, outline_color);
	render::get().line(box.x, box.y + box.h - 1, box.x, box.y + box.h * (1.f - calc_vertical), outline_color);
	render::get().line(box.x - 1, box.y + box.h, offset_horisontal + box.x + box.w * calc_horisontal, box.y + box.h, outline_color);
	render::get().line(box.x, box.y, box.x, box.y, outline_color);

	//another
	render::get().line(box.x, box.y, box.x, offsest_vertical + box.y + box.h * calc_vertical, color);
	render::get().line(box.x, box.y + box.h - 1, box.x, box.y + box.h * (1.f - calc_vertical), color);
	render::get().line(box.x + box.w - 1, box.y, box.x + box.w - 1, offsest_vertical + box.y + box.h * calc_vertical, color);
	render::get().line(box.x + box.w - 1, box.y + box.h - 1, box.x + box.w - 1, box.y + box.h * (1.f - calc_vertical), color);

	//another outline
	render::get().line(box.x - 1, box.y, box.x - 1, offsest_vertical + box.y + box.h * calc_vertical, outline_color);
	render::get().line(box.x - 1, box.y + box.h - 1, box.x - 1, box.y + box.h * (1.f - calc_vertical), outline_color);
	render::get().line(box.x + box.w, box.y, box.x + box.w, offsest_vertical + box.y + box.h * calc_vertical, outline_color);
	render::get().line(box.x + box.w, box.y + box.h - 1, box.x + box.w, box.y + box.h * (1.f - calc_vertical), outline_color);
