...
void legit_bot::find_best_point(CUserCmd* cmd, float fov_v)
{
	...
	if (g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].priority == 0)
	{
		for (auto bone : { HITBOX_HEAD, HITBOX_PELVIS, HITBOX_CHEST, HITBOX_UPPER_CHEST, HITBOX_STOMACH })
		{
			...
			auto fire_data = c_autowall::get().calculate_return_info(g_ctx.globals.eye_pos, bone_pos, g_ctx.local(), target);

			auto is_visible = false;

			Ray_t ray;
			ray.Init(g_ctx.local()->m_vecOrigin() + g_ctx.local()->m_vecViewOffset(), bone_pos);
			...
			if (!is_visible && !g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].awall_dmg)
				continue;

			if (!is_visible && g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].awall_dmg > 0 && fire_data.m_damage < g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].awall_dmg)
				continue;
