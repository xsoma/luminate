...
std::string get_config_dir()
{
	...
	if (SUCCEEDED(SHGetFolderPath(NULL, 0x001a, NULL, NULL, path)))
		folder = std::string(path) + crypt_str("\\luminate-project\\Configs\\");
...
void load_config()
{
	...
	eventlogs::get().add(crypt_str("Loaded ") + files.at(g_cfg.selected_config) + crypt_str("ini config"), false);
}
...
void save_config()
{
	...
	eventlogs::get().add(crypt_str("Saved ") + files.at(g_cfg.selected_config) + crypt_str("ini config"), false);
}
...
void remove_config()
{
	...
	eventlogs::get().add(crypt_str("Removed ") + files.at(g_cfg.selected_config) + crypt_str("ini config"), false);
...
void add_config()
{
	...
	if (g_cfg.new_config_name.find(crypt_str(".ini")) == std::string::npos)
		g_cfg.new_config_name += crypt_str(".ini");
...
void lua_edit(std::string window_name)
{
	...
	auto get_dir = [&]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
			file_path = std::string(path) + crypt_str("\\luminate-project\\Scripts\\");
	...
	if (ImGui::CustomButton(crypt_str("Save"), (crypt_str("Save") + window_name).c_str(), ImVec2(125 * (1.f + ((c_menu::get().dpi_scale - 1.0) * 0.5f)), 0)))
	{
		std::ofstream out;
	...
	if (!rage_section)
	{
		if (!child)
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE1_FIRST"), ImVec2(child_width, 0));
			{
				...
				ImGui::Checkbox(crypt_str("Automatic scope distance"), &g_cfg.ragebot.autoscope_distance);
	...
	else
	{
		...
		if (!child)
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE1_SECOND"), ImVec2(child_width, 0));
			{
				...
				padding(0, 3);
				draw_multicombo(crypt_str("Multipoints"), g_cfg.ragebot.weapon[hooks::rage_weapon].multipoints, multichlen, ARRAYSIZE(multichlen), preview);
...
void c_menu::draw_players(int child)
{
	...
	if (!child)
	{
		...
		ImGui::BeginChild(crypt_str("##ESP1_FIRST"), ImVec2(child_width, 0));
		{
			...
			ImGui::SliderFloat(crypt_str("Box horisontal"), &g_cfg.player.type[player].box_horisontal, 0.f, 100.f);
			ImGui::SliderFloat(crypt_str("Box vertical"), &g_cfg.player.type[player].box_vertical, 0.f, 100.f);
		...
		if (!misc_section)
		{
			...
			ImGui::BeginChild(crypt_str("##MISC2_FIRST"), ImVec2(child_width, 0));
			{
				...
				if (g_cfg.misc.airstrafe == 2) {
					ImGui::Checkbox(crypt_str("Movement keys"), &g_cfg.misc.wsadkeys);
					ImGui::SliderInt(crypt_str("Retrack speed"), &g_cfg.misc.retrack_speed, 0, 15, false);
				}
...
void c_menu::draw(bool is_open)
{
	...
	// start menu render
	ImGui::Begin(crypt_str("luminateproject"), nullptr, ImGuiWindowFlags_NoDecoration);
	{
		...
		ImGui::BeginChild(crypt_str("##footer"), ImVec2(x, footer_height));
		...
		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.f));
		ImGui::PushStyleVar(ImGuiStyleVar_Alpha, m_alpha);
		ImGui::Text(crypt_str("luminate-project"));
		ImGui::PopStyleVar();
		ImGui::PopStyleColor();
