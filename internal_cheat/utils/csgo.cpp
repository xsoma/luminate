...
IEngineVGui* C_CSGO::m_engine_gui() {
	if (!p_engine_gui)
		p_engine_gui = get_interface<IEngineVGui>(crypt_str("engine.dll"), crypt_str("VEngineVGui"));
...
ISoundServices* C_CSGO::m_soundservices() {
	if (!p_soundservices)
		p_soundservices = **reinterpret_cast<ISoundServices***>(util::FindSignature(crypt_str("engine.dll"), crypt_str("B9 ? ? ? ? 80 65 FC FE 6A 00") + 1));
