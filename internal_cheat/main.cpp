...
DWORD WINAPI main(PVOID base)
{
	g_ctx.signatures =
	{
		...
		crypt_str("B9 ? ? ? ? 80 65 FC FE 6A 00")
	};
	...
	main_folder = std::string(path) + crypt_str("\\luminate-project\\");
	configs_folder = std::string(path) + crypt_str("\\luminate-project\\Configs\\");
	scripts_folder = std::string(path) + crypt_str("\\luminate-project\\Scripts\\");
...
__forceinline void setup_render()
{
	...
	fonts[LOGS] = create_font(crypt_str("Verdana"), 12, FW_MEDIUM, FONTFLAG_DROPSHADOW | FONTFLAG_ANTIALIAS);
	fonts[ESP] = create_font(crypt_str("Smallest Pixel-7"), 11, FW_MEDIUM, FONTFLAG_OUTLINE);
	fonts[NAME] = create_font(crypt_str("Verdana"), 12, FW_MEDIUM, FONTFLAG_DROPSHADOW | FONTFLAG_ANTIALIAS);
	...
	fonts[INDICATORFONT] = create_font(crypt_str("Lucida Sans Unicode"), 14, FW_BOLD, FONTFLAG_DROPSHADOW | FONTFLAG_ANTIALIAS);
...
__forceinline void setup_hooks()
{
	...
	hooks::enginevgui_hook = new vmthook(reinterpret_cast<DWORD**>(m_engine_panel()));
	hooks::enginevgui_hook->hook_function(reinterpret_cast<uintptr_t>(hooks::hooked_enginevgui), 14);
	...
	hooks::soundservice_hook = new vmthook(reinterpret_cast<DWORD**>(util::FindSignature(crypt_str("engine.dll"), g_ctx.signatures.at(21).c_str()) + 1));
