...
void switch_color(ImVec4& color, const ImVec4 to, float deltatime)
{
    if (color.x != to.x)
    {
        if (color.x < to.x)
            color.x = ImMin(color.x + deltatime, to.x);
        else if (color.x > to.x)
            color.x = ImMax(to.x, color.x - deltatime);
    }

    if (color.y != to.y)
    {
        if (color.y < to.y)
            color.y = ImMin(color.y + deltatime, to.y);
        else if (color.y > to.y)
            color.y = ImMax(to.y, color.y - deltatime);
    }

    if (color.z != to.z)
    {
        if (color.z < to.z)
            color.z = ImMin(color.z + deltatime, to.z);
        else if (color.z > to.z)
            color.z = ImMax(to.z, color.z - deltatime);
    }

    if (color.w != to.w)
    {
        if (color.w < to.w)
            color.w = ImMin(color.w + deltatime, to.w);
        else if (color.w > to.w)
            color.w = ImMax(to.w, color.w - deltatime);
    }
}
...
bool ImGui::ContentTab(const char* label, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImVec4 text_hov = ImVec4(225 / 255.f, 225 / 255.f, 225 / 255.f, 1.f);
    ...
    if (hovered || active)
        switch_color(it_text->second, active ? text_act : text_hov, deltatime);
    else
        switch_color(it_text->second, text_dis, deltatime);
...
bool ImGui::MainTab(const char* label, ImFont* icon_font, const char* icon, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Render
    const ImVec4 text_dis = ImVec4(188 / 255.f, 188 / 255.f, 188 / 255.f, 1.f);
    const ImVec4 text_act = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 1.f);
    ...
    if (hovered || active)
        switch_color(it_text->second, text_act, deltatime);
    else
        switch_color(it_text->second, text_dis, deltatime);
    ...
    if (active)
        switch_color(it_icon->second, icon_act, deltatime);
    else
        switch_color(it_icon->second, icon_dis, deltatime);

    PushStyleColor(ImGuiCol_Text, it_icon->second);

    PushFont(icon_font);
    RenderTextClipped(bb.Min + ImVec2(size_arg.x * 0.05f, -3 * c_menu::get().dpi_scale), bb.Max, icon, NULL, &icon_size, ImVec2(0.f, 0.49f), &bb);
    PopFont();
...
bool ImGui::KeybindButton(const char* label, const char* unique_id, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags)
{
    ...
    if (clicked)
        switch_color(it_hcolor->second, hover_dis, deltatime);
    else
        switch_color(it_hcolor->second, hovered ? hover_dis : hover_act, deltatime);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    if (!hovered && !held)
        ...
        switch_color(it_hcolor->second, hover_act, deltatime);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
        switch_color(it_text->second, hovered ? text_dis : text_act, deltatime);
    else
        switch_color(it_text->second, text_dis, deltatime);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int* current_item)
{
    ...
    if (hovered || popup_open)
        switch_color(it_hover->second, hover_act, deltatime);
    else
        switch_color(it_hover->second, hover_dis, deltatime);
    ...
    if (popup_open)
        switch_color(it_arrow_col->second, arrow_act, deltatime);
    else
        switch_color(it_arrow_col->second, arrow_dis, deltatime);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, bool minimum_damage, float power)
{
    ...
    if (hovered || g.ActiveId == id)
        ...
        switch_color(it_hover->second, hover_off, deltatime);
    ...
    if (g.ActiveId == id)
        switch_color(it_grab->second, hover_act, deltatime);    
    else
        switch_color(it_grab->second, hover_dis, deltatime);
...
bool ImGui::CustomSelectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected)
        switch_color(it_hcolor->second, (hovered && !selected) ? text_hov : text_act, deltatime);      
    else
        switch_color(it_hcolor->second, text_dis, deltatime);
