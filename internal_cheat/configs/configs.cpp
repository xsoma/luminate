...
void C_ConfigManager::setup()
{
	...
	for (auto i = 0; i < 3; i++)
	{
		...
		setup_item(&g_cfg.player.type[i].box_horisontal, 0.f, std::to_string(i) + crypt_str("Player.box_horisontal"));
		setup_item(&g_cfg.player.type[i].box_vertical, 0.f, std::to_string(i) + crypt_str("Player.box_vertical"));
	...
	setup_item(&g_cfg.misc.buybot3, 0, crypt_str("Misc.buybot3"));
...
void C_ConfigManager::save(std::string config)
{
	...
	auto get_dir = [&folder, &file, &config]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
		{
			...
			file = std::string(path) + crypt_str("\\luminate-project\\Configs\\") + config;
		}
	};
...
void C_ConfigManager::config_files()
{
	...
	std::string path = folder + crypt_str("/*.ini");
	WIN32_FIND_DATA fd;
