...
#include "..\..\cheats\misc\slowwalk.h"
...
bool __stdcall hooks::hooked_createmove(float smt, CUserCmd* m_pcmd)
{
	...
	if (g_cfg.misc.buybot_enable && g_ctx.globals.should_buy)
	{
		...
		if (!g_ctx.globals.should_buy)
		{
			...
			if (g_cfg.misc.buybot3[BUY_HE])
				buy += crypt_str("buy hegrenade; ");

			if (g_cfg.misc.buybot3[BUY_MOLOTOV])
				buy += crypt_str("buy molotov; ");

			if (g_cfg.misc.buybot3[BUY_SMOKE])
				buy += crypt_str("buy smokegrenade; ");

			if (g_cfg.misc.buybot3[BUY_FLASH])
				buy += crypt_str("buy flashbang; ");

		/*	if (g_cfg.misc.buybot3[BUY_DECOY])
