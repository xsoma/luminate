...
void __stdcall hooks::hooked_enginevgui(int mode)
{
    ...
    enginevgui_hook->get_func_address<Paint_t>(14)(m_engine_panel(), mode);
	g_ctx.local((player_t*)m_entitylist()->GetClientEntity(m_engine()->GetLocalPlayer()), true);

	typedef void(__thiscall* start_drawing)(void*);
	typedef void(__thiscall* finish_drawing)(void*);

	static start_drawing start_draw = (start_drawing)util::FindSignature(crypt_str("vguimatsurface.dll"), crypt_str("55 8B EC 83 E4 C0 83 EC 38"));
	static finish_drawing end_draw = (finish_drawing)util::FindSignature(crypt_str("vguimatsurface.dll"), crypt_str("8B 0D ? ? ? ? 56 C6 05"));
