...
void __fastcall hooks::hooked_runcommand(void* ecx, void* edx, player_t* player, CUserCmd* m_pcmd, IMoveHelper* move_helper)
{
	...
	float backup_curtime = m_globals()->m_curtime;

	if (m_pcmd->m_tickcount >= (m_globals()->m_tickcount + int(1 / m_globals()->m_intervalpertick) + 8))
	{
	...
	if (m_pcmd->m_command_number == misc::get().shitfted_comm) {
		player->m_nTickBase() = (g_ctx.globals.tickbase_shift - g_ctx.globals.fixed_tickbase + 1);
...
next_cmd:
	...
	to_cmd.m_tickcount += 3 * (int)std::round(1.f / m_globals()->m_intervalpertick);
