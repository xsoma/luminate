			...
			if (!current_shot->latency)
			{
				...
				//		++g_ctx.globals.missed_shots[current_shot->last_target];
						...
						if (g_cfg.misc.events_to_log[EVENTLOG_HIT]) {
							eventlogs::get().add(crypt_str("shot missed due to bad resolver"));
							++g_ctx.globals.missed_shots[current_shot->last_target];
							lagcompensation::get().player_resolver[current_shot->last_target].last_side = (resolver_side)current_shot->side;
						}
						else if (current_shot->shot_info.backtrack_ticks > 10 && g_cfg.misc.events_to_log[EVENTLOG_HIT])
							eventlogs::get().add(crypt_str("shot missed due to bad bt record"));
					}
					else if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
					{
						current_shot->shot_info.result = crypt_str("Spread");
						++g_ctx.globals.missed_shots_spread[current_shot->last_target];

						if (current_shot->occlusion)
							eventlogs::get().add(crypt_str("shot missed due to occlusion"));
						else if (current_shot->impacts)
							eventlogs::get().add(crypt_str("shot missed due to prediction error ( unregistered shot )"));
						else if (current_shot->shot_info.hitchance == 100)
							eventlogs::get().add(crypt_str("shot missed due to prediction error ( max accuracy )"));
						else
							eventlogs::get().add(crypt_str("shot missed due to spread"));
					}
				}
			}
